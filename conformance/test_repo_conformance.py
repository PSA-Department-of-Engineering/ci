...
def test_app_oidc_gate_binds_the_apps_own_realm(repo_root: Path, deploy_repo: str) -> None:
    ...
    # and leaves the pod waiting forever. Without them, every app following the
    # documented pattern fails a gate the platform's own docs tell it to trip.
    allowed = {
        own_secret,
        f"keycloak-realm-{deploy_repo}",
        "keycloak-oidc",
        "keycloak-http",
        "keycloak-config-cli",
    }
